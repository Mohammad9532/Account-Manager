...
import { Coffee, TrendingDown, Plus, X, Calendar, Search, Filter, ArrowUpDown, Clock, Download, Upload, FileJson, Check, AlertCircle } from 'lucide-react';
...
const DailyExpensesView = () => {
    ...
    const handleExportExcel = () => {
        ...
        exportToExcel(data, `Daily_Expenses_${new Date().toISOString().split('T')[0]}`, 'Expenses', [
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 md:gap-0">
                ...
                <div className="flex items-center gap-2 w-full md:w-auto">
                    ...
                    >
                        ...
                        <span className="hidden lg:inline">{isSharing ? 'Generating...' : 'Report'}</span>
                    </button>
                    ...
                    <div className="relative w-full max-w-4xl bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                        <div className="p-6 border-b border-slate-800 flex items-center justify-between bg-slate-900/50">
                            <div className="flex items-center gap-3">
                                ...
                                <h3 className="text-xl font-bold text-white">Import Expenses ({importPreviewData.length})</h3>
                            </div>
                            ...
                            <table className="w-full text-left border-collapse">
                                <thead className="sticky top-0 bg-slate-900 text-slate-400 text-xs uppercase z-10">
                                    <tr>
                                        ...
                                        <th className="p-3 border-b border-slate-800 font-bold">Category</th>
                                        <th className="p-3 border-b border-slate-800 font-bold">Description</th>
                                        <th className="p-3 border-b border-slate-800 text-right font-bold">Amount</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-slate-800/50">
                                    {importPreviewData.map((t, idx) => (
                                        <tr key={idx} className="hover:bg-white/5 font-mono text-sm">
                                            <td className="p-3 text-slate-400">{new Date(t.date).toLocaleDateString()}</td>
                                            <td className="p-3 text-slate-300">{t.category}</td>
                                            <td className="p-3 text-slate-500 text-xs italic">{t.description}</td>
                                            <td className="p-3 text-right text-rose-400 font-bold">₹{t.amount.toLocaleString()}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                        ...
                        <div className="p-6 border-t border-slate-800 bg-slate-900/50 flex items-center justify-between">
                            <div className="flex items-center gap-2 text-amber-400 text-xs">
                                ...
                                <span>Verify dates and amounts before saving</span>
                            </div>
