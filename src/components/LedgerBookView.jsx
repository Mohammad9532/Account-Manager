...
const LedgerBookView = () => {
    ...
    const sharedAccounts = React.useMemo(
        () => safeAccounts.filter((a) => a && a.isShared),
        [safeAccounts],
    );
    ...
    const handleExportSummary = () => {
        ...
        const personalAccounts = safeAccounts.filter(
            (a) => a && !a.isShared && a.type === "Other",
        );
        ...
        exportToExcel(
            data,
            `Ledger_Summary_${new Date().toISOString().split("T")[0]}`,
            "Summary",
        ...
        try {
            ...
            const newTxns = data.flatMap((row, index) => {
                const amount = parseFloat(
                    row.Credit || row.Debit || row.Amount || 0,
                );
                if (amount <= 0) return [];
                ...
                const particulars =
                    ...
                    "Unknown Ledger";
                const type =
                    row.Credit > 0 || row.Type === TRANSACTION_TYPES.CREDIT
    ...
    return (
        <div className="space-y-4 animate-in fade-in slide-in-from-right-8 duration-300">
                ...
                <div>
                    <div>
                        ...
                        <p className="text-slate-500 dark:text-slate-400 text-sm">
                            Manage all your ledger entries
                        </p>
                    </div>
                </div>
            ...
            {/* Tabs */}
            <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-900/50 p-1 rounded-xl w-fit border border-slate-200 dark:border-slate-800">
                <button
                ...
                {sharedAccounts.length > 0 && (
                    ...
                    >
                        Shared with you
                        <span
                            className={`text-[10px] px-1.5 py-0.5 rounded-full ${activeTab === "shared" ? "bg-indigo-400/30" : "bg-slate-200 dark:bg-slate-700"}`}
                        >
                    ...
                    <div className="relative w-full max-w-4xl bg-slate-900 border border-slate-800 rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                        <div className="p-6 border-b border-slate-800 flex items-center justify-between bg-slate-900/50">
                            <div className="flex items-center gap-3">
                                ...
                                <h3 className="text-xl font-bold text-white">
                                    Global Import ({importPreviewData.length})
                                </h3>
                            </div>
                            <button
                            ...
                            <table className="w-full text-left border-collapse">
                                <thead className="sticky top-0 bg-slate-900 text-slate-400 text-xs uppercase z-10">
                                    <tr>
                                        ...
                                        <th className="p-3 border-b border-slate-800 font-bold text-right">
                                            Credit
                                        </th>
                        ...
                        <div className="p-6 border-t border-slate-800 bg-slate-900/50 flex items-center justify-between">
                            <div className="flex items-center gap-2 text-amber-400 text-xs">
                                ...
                                <span>
                                    Transactions will be linked to ledgers by
                                    name
                                </span>
                            </div>
