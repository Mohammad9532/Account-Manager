...
import { X, Plus, Trash2, Calendar, Percent, Receipt, CheckCircle, AlertCircle } from 'lucide-react';
import { useFinance } from '../context/FinanceContext';
import { TRANSACTION_TYPES, SCOPES } from '../utils/constants';

const ManageEMIsModal = ({ account, onClose }) => {
    const { accounts, updateAccount, addTransaction, formatCurrency } = useFinance();
    ...
    const handleAddEMI = async (e) => {
        ...
        try {
            ...
            // Simple calculation for display (Bank might differ)
            // Monthly = (Principal + Interest) / Months
            // Interest = Principal * (Rate/100) * (Months/12) -- Simple Interest approx
    ...
    const handleCloseEMI = async (index) => {
        if (!confirm('Are you sure you want to close this EMI? It will no longer block your limit.')) return;
    ...
    const updateInterestFromRate = (rate) => {
        ...
        const gAmount = iAmount * (parseFloat(billDetails.gstRate) || 0) / 100;
    ...
    const confirmBill = async (e) => {
        ...
        try {
            ...
            // We need addTransaction from context. 
            // I will add it to context destructuring in a moment.
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-finance-card border border-finance-border rounded-2xl w-full max-w-md p-6 relative shadow-2xl flex flex-col max-h-[90vh]">
                ...
                <h3 className="text-xl font-bold text-white mb-1">Manage EMIs</h3>
                <p className="text-sm text-slate-400 mb-6">{liveAccount.name}</p>
                        ...
                        <div className="space-y-3">
                            {allEMIs.length === 0 ? (
                                <div className="text-center py-8 text-slate-500 border-2 border-dashed border-slate-800 rounded-xl">
                                    ...
                                    <p>No EMIs found. Add one to get started.</p>
                                </div>
                            ) : (
                                allEMIs.map((emi, i) => (
                                    <div className="flex items-center justify-between p-3 bg-slate-900/50 rounded-xl border border-slate-800" key={i}>
                                        <div className="flex-1">
                                            <div className="flex items-center gap-2 mb-1">
                                                <h4 className="font-semibold text-white">{emi.name}</h4>
                                                {emi.status === 'Closed' && <span className="text-[10px] bg-slate-700 text-slate-300 px-1.5 py-0.5 rounded">Closed</span>}
                                            </div>
                                            <div className="space-y-1">
                                                <p className="text-xs text-slate-400">
                                                    Principal: <span className="text-emerald-400">{formatCurrency(emi.totalAmount)}</span>
                                                </p>
                                                <p className="text-xs text-slate-400">
                                                    Blocked: <span className="text-rose-400 font-medium">{formatCurrency(emi.remainingAmount)}</span>
                                                </p>
                                                <div className="flex items-center gap-2 mt-2">
                                                    ...
                                                    <div className="text-[10px] bg-emerald-500/10 text-emerald-400 px-2 py-0.5 rounded border border-emerald-500/20">
                                                        Paid: {emi.paidMonths || 0}/{emi.tenureMonths}
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                        ...
                        <form onSubmit={handleAddEMI} className="space-y-4">
                            <div>
                                ...
                                <input
                                    type="text"
                                    value={newEMI.name}
                                    onChange={e => setNewEMI(prev => ({ ...prev, name: e.target.value }))}
                            ...
                            <div>
                                <label className="block text-xs text-slate-400 mb-1">Principal Amount (Blocked)</label>
                                <div className="relative">
                                    <span className="absolute left-4 top-3.5 text-slate-500">₹</span>
                                    <input
                                        type="number"
                                        value={newEMI.totalAmount}
                                        onChange={e => setNewEMI(prev => ({ ...prev, totalAmount: e.target.value }))}
                                        placeholder="0.00"
                                ...
                                <p className="text-[10px] text-indigo-400 mt-1">This amount will be blocked from your credit limit.</p>
                            </div>


                            {/* Tenure Selection */}
                            <div>
                                <label className="block text-xs text-slate-400 mb-2">Tenure (Months)</label>
                                <div className="flex flex-wrap gap-2">
                                    {tenureOptions.map(t => (
                                        <button
                                            key={t}
                                            type="button"
                                            onClick={() => setNewEMI(prev => ({ ...prev, tenureMonths: t }))}
                                            className={`px-3 py-2 rounded-lg text-xs font-medium border transition-all ${parseInt(newEMI.tenureMonths) === t
                                    ...
                                    <div className="relative w-20">
                                        <input
                                            ...
                                            onChange={e => setNewEMI(prev => ({ ...prev, tenureMonths: e.target.value }))}
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        ...
                                        <input
                                            ...
                                            onChange={e => setNewEMI(prev => ({ ...prev, interestRate: e.target.value }))}
                                            placeholder="0"
                                            className="w-full pl-8 bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-indigo-500"
                                        />
                                    </div>
                                ...
                                <div>
                                    ...
                                    <div className="relative">
                                        ...
                                        <input
                                            type="number"
                                            value={newEMI.gst}
                                            onChange={e => setNewEMI(prev => ({ ...prev, gst: e.target.value }))}
                                            placeholder="18"
                                            className="w-full pl-8 bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-indigo-500"
                                        />
                                    </div>
            ...
            {billingEMI && (
                <div className="absolute inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm rounded-2xl animate-in fade-in duration-200">
                    <div className="bg-slate-900 border border-slate-700 rounded-xl w-full max-w-sm p-5 shadow-2xl">
                        <h4 className="font-bold text-white mb-4">Bill Installment</h4>
                        <div className="space-y-3">
                            <div>
                            ...
                            <div>
                                <label className="text-xs text-slate-400">Principal Component (Releases Block)</label>
                                <input
                                    type="number"
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={e => updateInterestFromRate(e.target.value)}
                                        className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-indigo-500"
                                    />
                                </div>
                                <div>
                                    <label className="text-xs text-slate-400">Interest Amt</label>
                                    <input
                                        type="number"
                                        value={billDetails.interest}
                                        onChange={e => setBillDetails({ ...billDetails, interest: e.target.value })}
                                        className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-indigo-500"
                                    />
                                </div>
                            </div>
                            <div className="grid grid-cols-2 gap-3">
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={e => updateGSTFromRate(e.target.value)}
                                        className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-indigo-500"
                                    />
                                </div>
                                <div>
                                    <label className="text-xs text-slate-400">GST Amt</label>
                                    <input
                                        type="number"
                            ...
                            <div className="py-2 mt-2 border-t border-slate-800">
                                <div className="flex justify-between items-center text-sm font-bold">
                                    ...
                                    <span className="text-emerald-400">
                                        {formatCurrency((parseFloat(billDetails.amount) || 0) + (parseFloat(billDetails.interest) || 0) + (parseFloat(billDetails.gst) || 0))}
                                    </span>
                                </div>
                            </div>
