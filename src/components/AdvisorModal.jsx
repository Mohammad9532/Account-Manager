...
const AdvisorModal = ({ isOpen, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-finance-bg border border-finance-border rounded-2xl w-full max-w-lg p-8 relative shadow-2xl animate-in zoom-in-95 duration-200">
                <button
                    onClick={handleClose}
                ...
                {step === 'form' ? (
                    <>
                        <h2 className="text-2xl font-bold text-finance-text mb-2">Get your money system reviewed</h2>
                        <p className="text-slate-500 dark:text-slate-400 mb-6">Answer 3 quick questions to help us match you with the right expert.</p>

                        <form onSubmit={handleSubmit} className="space-y-6">

                            {/* Question 1: Role */}
                            <div>
                                <label className="block text-sm font-semibold text-slate-600 dark:text-slate-400 mb-2">What do you do?</label>
                                    ...
                                    {['Currency dealer', 'Trader', 'Shop', 'Service business', 'Other'].map(opt => (
                                        <button
                                            ...
                                            className={`p-3 rounded-lg text-sm font-medium border text-left transition-all ${formData.role === opt
                                                ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-500/10 text-emerald-800 dark:text-emerald-400 ring-1 ring-emerald-500'
                                                : 'border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 hover:border-emerald-200 dark:hover:border-emerald-800 hover:bg-emerald-50/50 dark:hover:bg-emerald-500/5'
                                                }`}
                                        >
                                            {opt}
                                        </button>
                    ...
                    <div className="text-center py-12 animate-in fade-in zoom-in duration-300">
                        ...
                        <h2 className="text-2xl font-bold text-finance-text mb-2">Request Received!</h2>
