...
const LedgerDetailView = ({
    ...
    const handleShare = async () => {
        ...
        if (isMobile) {
            ...
            try {
                ...
                toast.error(
                    "Share failed. Please try downloading from Desktop.",
                );
            } finally {
                setIsSharing(false);
            }
    ...
    const handleImportExcel = async (e) => {
        try {
            ...
            toast.error("Error parsing Excel file. Please use the template.");
        }
    ...
    const sharedLimitStats = useMemo(() => {
        ...
        if (accountDetails.linkedAccountId) {
            parentAccount = accounts.find(
                (a) => a._id === accountDetails.linkedAccountId,
            );
            isChild = true;
        }
        ...
        const familyAccounts = accounts.filter(
            (a) =>
                a._id === parentAccount._id ||
                a.linkedAccountId === parentAccount._id,
        );
        ...
        const totalUsed = familyAccounts.reduce(
        ...
        const utilization =
            sharedLimit > 0 ? (Math.abs(totalUsed) / sharedLimit) * 100 : 0;
    ...
    const displayBalance = billingStats
        ? billingStats.totalOutstanding
        : finalBalance;
    ...
    const handleDeleteAccount = async () => {
        if (
            window.confirm(
                `Are you sure you want to delete account "${ledgerName}"? This cannot be undone.`,
            )
        ) {
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-right-8 duration-300">
                    ...
                    <div className="flex items-center gap-2 border-r border-slate-800 pr-4 mr-2">
                        ...
                        {isAccount &&
                            (accountDetails.type === "Other" ||
                                accountDetails.transactionCount === 0 ||
                        ...
                        {sharedLimitStats ? (
                            <>
                                <div className="p-4 bg-slate-900/50 rounded-xl border border-blue-500/20 relative overflow-hidden group">
                                    ...
                                    <div className="text-blue-400 text-xs uppercase font-bold mb-1">
                                        Shared Used
                                    </div>
                                    <div className="text-lg font-mono font-bold text-rose-400">
                                        {sharedLimitStats.utilization.toFixed(
                                            1,
                                        )}
                                        %
                ...
                <div className="col-span-2 flex flex-col gap-2">
                    ...
                    <div className="flex items-center gap-2">
                        <div className="flex-1">
                            <label className="text-[10px] text-slate-500 uppercase font-bold ml-1 mb-1 block">
                                From
                            </label>
                            <input
                                type="date"
                    ...
                    <tbody className="divide-y divide-slate-800/50">
                        {ledgerTransactions.map((t) => {
                            ...
                            if (isLinked) {
                                const primaryAcc = accounts.find(
                                    (a) =>
                                        String(a._id) === String(t.accountId),
                                );
                                const linkedAcc = accounts.find(
                                    (a) =>
                                        String(a._id) ===
                                        String(t.linkedAccountId),
                                );
                                const internalTypes = [
                                    "Bank",
                                    "Cash",
                            ...
                            const isCredit =
                                effectiveType === TRANSACTION_TYPES.CREDIT;
                ...
                {ledgerTransactions.map((t) => {
                    ...
                    return (
                        <div
                            ...
                            className={`p-3.5 rounded-xl border ${isSelected ? "border-blue-500 bg-blue-500/5" : "border-slate-800 bg-slate-900/50"} active:scale-[0.98] transition-all`}
                        >
                            <div className="flex justify-between items-start mb-2">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <span
                                            className="text-xs text-slate-500 font-mono block"
                                            suppressHydrationWarning
                                        >
                                            {new Date(
                                                t.date,
                            ...
                            {t.description && t.description !== ledgerName && (
                                <p className="text-sm text-slate-400 pl-8 mb-2 border-l-2 border-slate-800 ml-1">
                                    {t.description}
                                </p>
                            )}
                        </div>
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-slate-400 text-sm">
                                        Review {importPreviewData.length}{" "}
                                        records before adding to {ledgerName}
                                    </p>
                                </div>
                            </div>
                            <button
                        ...
                        <div className="flex-1 overflow-auto p-4 md:p-6">
                            ...
                            <div className="md:hidden space-y-3">
                                {importPreviewData.map((t, idx) => (
                                    <div
                                        key={idx}
                                        className="bg-slate-950 rounded-xl border border-slate-800 p-4 flex justify-between items-center"
                                    >
                                        <div>
                                            <div
                                                className="text-xs text-slate-500 font-mono mb-1"
                                                suppressHydrationWarning
                                            >
                                                {new Date(
                                                    t.date,
            ...
            {showSettleModal && (
                <SettleCardModal
                    ...
                    initialAmount={Math.abs(
                        billingStats?.currentDue || finalBalance,
                    )}
