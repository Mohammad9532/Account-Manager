...
import { X, ArrowRightLeft, Calculator, Banknote, RefreshCcw, UserPlus, ArrowRight, Wallet, Receipt } from 'lucide-react';

const AddDealerTransactionModal = ({ isOpen, onClose, dealer, onAddTransaction }) => {
    // Modes: 'trade' (Sell to Customer), 'payment', 'receipt'
    const [activeTab, setActiveTab] = useState('trade');

    // Rate Config
    const [rateType, setRateType] = useState('inverse'); // 1 AED = X FC (Inverse) or 1 FC = X AED (Direct)
    ...
    // Amounts
    const [customerRate, setCustomerRate] = useState(''); // Rate we sell at
    const [collectionAmount, setCollectionAmount] = useState(''); // AED we collect from customer (Revenue)
    const [foreignAmountInput, setForeignAmountInput] = useState('');

    const [dealerRate, setDealerRate] = useState(''); // Rate we buy from Dealer at (Cost)

    // Payment/Receipt Amounts
    // Payment/Receipt Amounts
    const [paymentAmount, setPaymentAmount] = useState('');
    const [instantReceiptAmount, setInstantReceiptAmount] = useState('');
    ...
    useEffect(() => {
        if (dealer) {
            const currency = dealer.defaultCurrency || 'INR';
    ...
    const handleCollectionChange = (val) => {
        ...
        const collected = parseFloat(val) || 0;
        const cRate = parseFloat(customerRate) || 0;

        if (cRate) {
            let foreign = 0;
            if (rateType === 'inverse') {
                foreign = collected * cRate;
            } else {
                foreign = collected / cRate;
            }
            setForeignAmountInput(foreign ? foreign.toFixed(2) : '');
        }
    };
    ...
    const handleForeignChange = (val) => {
        ...
        if (cRate) {
            let collected = 0;
            if (rateType === 'inverse') {
                collected = foreign / cRate;
            } else {
                collected = foreign * cRate;
            }
            setCollectionAmount(collected ? collected.toFixed(2) : '');
        }
    };

    const handleRateChange = (val) => {
        setCustomerRate(val);
        // If we change rate, update Foreign Amount based on Collection (default)
        // Or update Collection based on Foreign? Usually Collection is fixed (Customer pays X AED).
        const cRate = parseFloat(val) || 0;
        const collected = parseFloat(collectionAmount) || 0;
    ...
    // Recalculate Dealer Cost & Profit whenever relevant values change
    useEffect(() => {
        if (activeTab === 'trade') {
            const collected = parseFloat(collectionAmount) || 0;
            const foreign = parseFloat(foreignAmountInput) || 0;
            const dRate = parseFloat(dealerRate) || 0;
            let cost = 0;

            if (foreign && dRate) {
                if (rateType === 'inverse') {
                    // Inverse: FC / Rate = AED Cost
                    cost = foreign / dRate;
                } else {
                    // Direct: FC * Rate = AED Cost
                    cost = foreign * dRate;
                }
            }
            ...
            setCalc({
                ...
                dealerCost: cost,
                profit: cost ? (collected - cost) : 0
            });
        }
    }, [collectionAmount, foreignAmountInput, dealerRate, rateType, activeTab]);
        ...
        try {
            if (activeTab === 'trade') {
                ...
                const dealerTxn = {
                    ...
                    description: description || `Trade - Cust: ${customers.find(c => c._id === selectedCustomerId)?.name || newCustomerName}`
                };
                ...
                if (custId) {
                    ...
                    await fetch(`/api/currency-customers/${custId}`, {
                        method: 'PUT',
                        body: JSON.stringify({ $push: { transactions: customerTxn } }),
                        headers: { 'Content-Type': 'application/json' }
                    });

                    // 3. Instant Receipt (if any)
                    if (instantReceiptAmount && parseFloat(instantReceiptAmount) > 0) {
                        const receiptTxn = {
                            date,
                        ...
                        await fetch(`/api/currency-customers/${custId}`, {
                            method: 'PUT',
                            body: JSON.stringify({ $push: { transactions: receiptTxn } }),
                            headers: { 'Content-Type': 'application/json' }
                        });
                    }
                }
                ...
                <div className="p-6 border-b border-slate-700 flex justify-between items-center bg-slate-800/50">
                    <div>
                        <h2 className="text-xl font-bold text-white flex items-center gap-2">
                            {activeTab === 'trade' && <ArrowRightLeft className="w-5 h-5 text-emerald-400" />}
                            {activeTab === 'payment' && <Wallet className="w-5 h-5 text-rose-400" />}
                            {activeTab === 'receipt' && <Receipt className="w-5 h-5 text-blue-400" />}
                            {activeTab === 'trade' ? 'Sell to Customer' : activeTab === 'payment' ? 'Make Payment' : 'Receive Money'}
                        </h2>
                        <p className="text-sm text-slate-400 mt-1">Dealer: {dealer?.name}</p>
                    </div>
                        ...
                        <div className="space-y-5 animate-in slide-in-from-bottom-2">
                            ...
                            <div className="flex justify-between items-center">
                                <span className="text-xs text-slate-500 bg-slate-800 px-2 py-1 rounded">
                                    Logic: {rateType === 'inverse' ? 'AED x Rate = FC' : 'AED / Rate = FC'}
                                </span>
                                <button
                                    type="button"
                                    onClick={() => setRateType(prev => prev === 'direct' ? 'inverse' : 'direct')}
                                ...
                                {!showNewCustomerInput ? (
                                    <div className="flex gap-2">
                                        <select
                                            ...
                                            onChange={(e) => {
                                                const cw = customers.find(c => c._id === e.target.value);
                                                setSelectedCustomerId(e.target.value);
                                                setSelectedCustomerBalance(cw ? cw.balance : 0);
                                            }}
                            ...
                            <div className="bg-slate-800 p-4 rounded-xl border border-slate-700/50 space-y-4">
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="relative">
                                        ...
                                        />
                                        <span className="absolute right-4 top-2.5 text-xs text-slate-500 font-mono font-bold pt-1">{foreignCurrency}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            {(selectedCustomerId || showNewCustomerInput) && (
                                <div className="bg-slate-800 p-4 rounded-xl border border-slate-700/50 space-y-3">
                                    <div className="flex justify-between items-center text-sm">
                                        ...
                                        <span className={`font-mono font-bold ${selectedCustomerBalance > 0 ? 'text-emerald-400' : 'text-slate-500'}`}>
                                            {selectedCustomerBalance?.toFixed(2)} AED
                                        </span>
                                    </div>
                                    <div className="flex justify-between items-center text-sm">
                                        <span className="text-slate-400">+ This Trade</span>
                                        <span className="font-mono font-bold text-white">
                                            {collectionAmount ? parseFloat(collectionAmount).toFixed(2) : '0.00'} AED
                                        </span>
                                    </div>
                                    ...
                                    <div className="flex justify-between items-center">
                                        ...
                                        <span className="font-mono font-bold text-lg text-emerald-400">
                                            {((parseFloat(selectedCustomerBalance) || 0) + (parseFloat(collectionAmount) || 0)).toFixed(2)} AED
                                        </span>
                                    </div>
                                    ...
                                    <div className="pt-2">
                                        ...
                                        <input
                                            ...
                                            onChange={(e) => setInstantReceiptAmount(e.target.value)}
                            ...
                            <div className="flex justify-center text-slate-500"><ArrowRight className="w-5 h-5 rotate-90" /></div>
                            ...
                            <div className="bg-slate-800 p-4 rounded-xl border border-slate-700/50 space-y-4">
                                <div className="space-y-2">
                                    ...
                                    <div className="relative">
                                        ...
                                        <input
                                            ...
                                            required
                                            value={dealerRate}
                                            onChange={(e) => setDealerRate(e.target.value)}
                                ...
                                <div className="flex justify-between items-center pt-2 border-t border-slate-700/50">
                                    <span className="text-xs text-slate-500">Pay to Dealer</span>
                            ...
                            <div className="bg-slate-900 p-4 rounded-xl border border-slate-700 flex justify-between items-center">
                                ...
                                <span className={`text-xl font-bold font-mono ${calc.profit >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                        ...
                        <div className="space-y-4 animate-in slide-in-from-bottom-2">
                            <div className="space-y-2">
                                <label className="text-sm font-bold text-white">
                                    {activeTab === 'payment' ? 'Payment Amount (AED)' : 'Receipt Amount (AED)'}
                                </label>
                                <div className="relative">
                                    <Banknote className={`absolute left-3 top-2.5 w-5 h-5 ${activeTab === 'payment' ? 'text-rose-500' : 'text-blue-500'}`} />
                                    <input
                                        type="number"
                                        step="0.01"
                                        required
