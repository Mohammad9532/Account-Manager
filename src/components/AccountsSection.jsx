...
const AccountsSection = ({ onAccountClick }) => {
    const { accounts, createAccount, deleteAccount, formatCurrency } =
        useFinance();
    ...
    const grouped = React.useMemo(() => {
        const primaryMoneySources = accounts.filter((a) =>
        ...
        if (newAccount.type === "Credit Card") {
            if (!newAccount.linkedAccountId) {
                ...
                const parent = accounts.find(
                    (a) => a._id === newAccount.linkedAccountId,
                );
    ...
    const handleResetData = async () => {
        if (
            window.confirm(
                "WARNING: This will delete ALL accounts and transactions. This cannot be undone. Are you sure?",
            )
        ) {
            if (
                window.confirm(
                    "Double check: Are you absolutely sure you want to reset all data for this user?",
                )
            ) {
                try {
                    ...
                    if (res.ok) {
                        ...
                        toast.error(
                            `Failed to reset data: ${errorData.error || "Unknown error"}`,
                        );
                    }
                } catch (error) {
    ...
    return (
        <div className="space-y-4">
            <div className="flex items-center justify-between">
                <h3 className="text-lg font-bold text-finance-text">Money Sources</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                {grouped.singles.map((account) => {
                    ...
                    return (
                        <AccountCard
                            ...
                            onAddConnected={
                                isCC
                                    ? () => handleAddConnected(account._id)
                                    : undefined
                            }
                ...
                {grouped.groups.map((family) => {
                    ...
                    const totalUsed = members.reduce(
                        (sum, member) =>
                            sum + Math.abs(parseFloat(member.balance) || 0),
                        0,
                    );
                    ...
                    const totalEMIBlocked = members.reduce((sum, member) => {
                        ...
                        const blocked = memberEMIs
                            ...
                            .reduce(
                                (s, e) =>
                                    s + (parseFloat(e.remainingAmount) || 0),
                                0,
                            );
                        return sum + blocked;
                    }, 0);
                    ...
                    const realAvailable =
                        headLimit - totalUsed - totalEMIBlocked;

                    return (
                        <div
                            key={family.head._id}
                            className="relative col-span-1 md:col-span-2 lg:col-span-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 p-4 border border-indigo-500/30 rounded-2xl bg-indigo-500/5"
                        >
                            ...
                            <div className="col-span-full flex flex-wrap items-center justify-between gap-4 mb-2">
                                ...
                                <div className="flex items-center gap-4 text-xs md:text-sm">
                                    <div className="text-slate-500 dark:text-slate-400">
                                        Limit:{" "}
                                        <span className="text-finance-text font-mono">
                                            {formatCurrency(headLimit)}
                                        </span>
                                    </div>
                            ...
                            <AccountCard
                                ...
                                isHead={true}
                                onAddConnected={() =>
                                    handleAddConnected(family.head._id)
                                }
                                onManageEMIs={setManagingEMIs}
                            />
            ...
            {showAdd && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-finance-card border border-finance-border rounded-2xl w-full max-w-sm p-6 relative animate-in zoom-in-95 duration-200 shadow-2xl">
                        ...
                        <h3 className="text-xl font-bold text-finance-text mb-6">
                            Add New Account
                        </h3>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            {!newAccount.linkedAccountId && (
                                <div>
                                    ...
                                    <select
                                        ...
                                        className="w-full bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 text-finance-text focus:outline-none focus:border-emerald-500 appearance-none"
                                    >
                                        <option value="Bank">
                                            Bank Account
                                        </option>
                            ...
                            {newAccount.type === "Credit Card" &&
                                ...
                                <div>
                                    ...
                                    />
                                    <p className="text-[10px] text-slate-500 mt-1">
                                        Enter the available limit shown on this
                                        card.
                                    </p>
                                </div>
                            ) : (
...
const AccountCard = ({
    ...
    return (
        ...
        >
            <div
                className={`absolute top-2 right-2 ${isHead ? "opacity-100" : "opacity-0 group-hover:opacity-100"} transition-opacity flex items-center gap-1`}
            >
                {isHead && (
                    <button
                        onClick={(e) => {
                ...
                {(account.transactionCount === 0 ||
                    !["Cash", "Credit Card"].includes(account.type)) && (
                        <button
                            onClick={(e) => {
                                e.stopPropagation();
            ...
            <div className="flex items-center gap-3 mb-3">
                <div
                    className={`p-2 ${isConnected ? "bg-sky-500/10" : "bg-slate-200 dark:bg-slate-900/50"} rounded-lg shrink-0`}
                >
                ...
                <div>
                    ...
                    <p className="text-finance-text font-medium truncate w-[120px]">
                        {account.name}
                    </p>
                </div>
            </div>
            ...
            <div className="mt-2">
                <span
                    className={`text-lg font-mono font-bold ${account.balance >= 0 ? "text-emerald-400" : "text-rose-400"}`}
                >
