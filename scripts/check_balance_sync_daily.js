...
async function checkSyncWithDaily() {
    ...
    // Find all DailyExpenses for this user
    const dailyExpenses = await DailyExpense.find({ userId }).sort({ date: -1 });

    console.log(`Total transactions: ${txs.length}`);
    console.log(`Total daily expenses: ${dailyExpenses.length}`);
    ...
    for (const tx of txs) {
        ...
        if (isPrimary || isLinked) {
            ...
            if (isPrimary) {
                ...
                console.log(`[Tx-Primary] Date: ${tx.date?.toISOString().substring(0,10)}, Type: ${tx.type}, Category: ${tx.category}, Amt: ${tx.amount}, Impact: ${impact}, Desc: ${tx.description}, txId: ${tx._id}`);
            } else {
                ...
                computedBalance += impact;
                console.log(`[Tx-Linked]  Date: ${tx.date?.toISOString().substring(0,10)}, Type: ${tx.type}, Category: ${tx.category}, Amt: ${tx.amount}, BaseImpact: ${baseImpact}, LinkedImpact: ${impact}, PrimaryType: ${primaryType}, Desc: ${tx.description}, txId: ${tx._id}`);
            }
        }
    }

    // Process DailyExpenses
    for (const exp of dailyExpenses) {
        const expAccId = exp.accountId ? exp.accountId.toString() : '';
        if (expAccId === ccIdStr) {
            matchCount++;
            const impact = exp.balanceImpact || 0;
            computedBalance += impact;
            console.log(`[Daily-Expense] Date: ${exp.date?.toISOString().substring(0,10)}, Type: ${exp.type}, Category: ${exp.category}, Amt: ${exp.amount}, Impact: ${impact}, Desc: ${exp.description}, expId: ${exp._id}`);
        }
    }

    console.log(`\nMatched documents: ${matchCount}`);
